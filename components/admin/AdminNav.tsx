...
import { LayoutDashboard, ImageIcon, LogOut, Menu, Package, Tag, Tags, UserRound, UsersRound, X } from "lucide-react";
...
function UserBadge({ profile, onNavigate }: { profile: StaffProfile; onNavigate?: () => void }) {
  const pathname = usePathname();
  const active = pathname.startsWith("/admin/perfil");
  return (
    <Link
      href="/admin/perfil"
      onClick={onNavigate}
      aria-current={active ? "page" : undefined}
      className={cn(
        "flex items-center gap-3 rounded-brand px-3 py-2.5 text-xs transition-colors",
        active ? "bg-brand-white/10" : "hover:bg-brand-white/5",
      )}
    >
      <UserRound className="h-4 w-4 shrink-0 text-brand-gray-400" aria-hidden="true" />
      <span className="flex min-w-0 flex-col gap-0.5">
        <span className="font-semibold text-brand-white">{profile.nome ?? "Equipe"}</span>
        <span className="truncate text-brand-gray-400">Editar perfil</span>
      </span>
    </Link>
  );
}
      ...
      {mobileOpen && (
        <div className="border-b border-brand-gray-600/30 bg-brand-black px-4 py-3 md:hidden">
          ...
          <div className="mt-3 flex flex-col gap-2 border-t border-brand-gray-600/30 pt-3">
            <UserBadge profile={profile} onNavigate={() => setMobileOpen(false)} />
            <form action={logout}>
              <button
                type="submit"
